...
onready var Jet2 = get_node("Jet2")


var bullet_scene = preload("res://scenes/player/weapons/ArrowBullet.tscn")
...
func _process(delta: float) -> void:
	...
	if Input.get_action_strength("move") > 0:
		...
		if Jet.emitting == false:
			$JetNoise.play()
			Jet.set_emitting(true)
			Jet2.set_emitting(true)
	else:
		$JetNoise.stop()
		Jet.set_emitting(false)
		Jet2.set_emitting(false)
